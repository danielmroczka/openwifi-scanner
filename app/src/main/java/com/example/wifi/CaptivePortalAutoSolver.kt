...
class CaptivePortalAutoSolver(
    ...
    private suspend fun trySolveViaHttp(): Boolean = withContext(Dispatchers.IO) {
        try {
            // 1. Hit connectivity check – expect 204 (OK) or a redirect to the portal
            ...
            if (code == 204) { checkConn.disconnect(); return@withContext true }
            ...
            // 2. Load the portal page (keep cookies for the session)
            ...
            submitConn.responseCode          // execute
    ...
    private fun parseInput(tag: String): String? {
        ...
        if (type == "password") return null                       // never guess passwords
