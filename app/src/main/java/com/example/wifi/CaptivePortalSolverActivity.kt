...
private fun CaptivePortalSolverScreen(onClose: () -> Unit) {
    ...
    // Monitor connectivity — auto-close when internet is validated
    LaunchedEffect(Unit) {
                ...
                WebView(ctx).apply {
                    ...
                    webViewClient = object : WebViewClient() {
                        override fun onPageFinished(view: WebView?, url: String?) {
                            ...
                            view?.let { wv ->
                                injectSolverJs(wv)
                                // Retry after delay for dynamic pages
                                scope.launch {
                                    delay(3_000)
                                    injectSolverJs(wv)
                                }
...
private fun injectSolverJs(webView: WebView) {
    ...
    (function() {
        ...
        // 2. Select first radio in each group
        var radioGroups = {};
        ...
        // 3. Find and click accept / connect buttons (multi-language)
        var keywords = [
        ...
        // 4. Fallback: common CSS selectors
        if (!clicked) {
            var fallbacks = [
        ...
        // 5. Last resort: submit first form
        if (!clicked) {
